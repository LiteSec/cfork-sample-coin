...
//
// Bootup the Masternode, look for a 10000 Cforksamplecoin input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
		...
		if (pwalletMain->GetBalance() < Params().MasternodeCollateralAmt()*COIN) {
			LogPrintf("CActiveMasternode::ManageStateInitial -- %s: Wallet balance is < 1,000 CFRK\n", GetStateString());
		}
